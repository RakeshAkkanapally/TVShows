...
const reducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case FILTER_SHOWS:
            ...
            let shows = rating === "All" ? action.payload.data : action.payload.data.filter((show) => { return Math.floor(show.rating.average) === parseInt(rating) });
            shows = genre.length === 0 ? shows : shows.filter((show) => {
                let found = false;
            ...
            let filteredShows= shows.sort((a,b)=>parseFloat(b.rating.average)-parseFloat(a.rating.average))
