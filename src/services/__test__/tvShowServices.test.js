...
describe("verify all service calls", () => {
  ...
  it("verify search a show response", async () => {
    ...
    const result = await service.getSearchShow("girls");
  ...
  it("verify a single show response", async () => {
    ...
    const result = await service.getSingleShow("Breaking Bad");
    expect(result.name).toBeDefined();
    ...
    const data = [
      {
        ...
        summary: "",
        _links: { self: { href: "https://api.tvmaze.com/seasons/1" } },
      },
    ];

    axios.get.mockImplementationOnce(() => Promise.resolve(data));

    const result = await service.getSeasons("1");
